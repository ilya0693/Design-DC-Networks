...
**_Примечание_**: Значение `Z` зависимо от значения `_X_` и, в некоторых случаях, зависимо от значения `_Y_`. Если `X=1`, то значение `Z` варьируется **от 0 до 9** и не зависит от значения `Y`. Если `X=2`, то `Z` варьируется от **от 0 до 9**, если значение `Y= от 0 до 4`. Если `X=2`, а `_Y_=5` то `Z` варьируется от **от 0 до 5**.
